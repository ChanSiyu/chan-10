...
#include "freeRunMotion.H"
#include "freeRunSolver.H"
...
namespace Foam
{
    namespace REL
    {
        defineTypeNameAndDebug(freeRunMotion, 0);
...
void Foam::REL::freeRunMotion::applyRestraints()
{
...
Foam::REL::freeRunMotion::freeRunMotion(const dictionary& dict, const dictionary& stateDict)
{
    motionState_(),
    motionState0_(),
    restraints_(),
...
Foam::REL::freeRunMotion::freeRunMotion
(
...
:
    ...
    solver_(freeRunSolver::New(dict.subDict("solver"), *this))
{
    addRestraints(dict);
...
void Foam::REL::freeRunMotion::addRestraints
(
    const dictionary& dict
)
{
...
void Foam::REL::freeRunMotion::addConstraints
(
    const dictionary& dict
)
{
...
void Foam::REL::freeRunMotion::updateAcceleration
(
...
void Foam::REL::freeRunMotion::update
(
    bool firstIter,
    const vector& fGlobal,
    const vector& tauGlobal,
...
void Foam::REL::freeRunMotion::status() const
{
    Info<< "Free-Run rigid body motion" << nl
...
Foam::tmp<Foam::pointField> Foam::REL::freeRunMotion::transform
(
...
bool Foam::REL::freeRunMotion::read()
{
    if (RELModel::read())
    {
        return true;
...
Foam::REL::freeRunMotion::Su()
{
    scalar deltaT = db().time().deltaT().value();

    force_.value() = vector(1, 0, 0); // for test
    translationalAcceleration_.value() = force_.value() / mass_;
    translationalVelocity_.value() = translationalVelocity_.value() + translationalAcceleration_.value() * deltaT;
    Info<< "Current force value: " << force_.value() << endl;
    Info<< "Current translational   velocity   value: " << translationalVelocity_.value() << endl;
    Info<< "Current translational acceleration value: " << translationalAcceleration_.value() << endl;

    moment_.value() = vector(1, 0, 0); // for test
    angularAcceleration_.value() = moment_.value() / momentOfInertia_;
    angularVelocity_.value() = angularVelocity_.value() + angularAcceleration_.value() * deltaT;
    Info<< "Current moment value: " << moment_.value() << endl;
    Info<< "Current angular velocity     value: " << angularVelocity_.value() << endl;
    Info<< "Current angular Acceleration value: " << angularAcceleration_.value() << endl;

    axis_ = angularVelocity_.value();
    axis_ /= mag(axis_);
    angle_.value() = angle_.value() + angularVelocity_.value() * deltaT;
